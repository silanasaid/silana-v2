...
END:VCARD
		...
		downloadM: {
			...
			async value(m, type, saveToFile) {
				...
				const res = saveToFile && fs.existsSync(filename) ? filename : buffer;
				mediaCache.set(mediaKey, res);
				return res;
			},
			enumerable: true,
		},
...
export function serialize() {
	...
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		chat: {
			get() {
				return this.conn?.getJid(this.key?.remoteJidAlt?.endsWith('@s.whatsapp.net') ? this.key.remoteJidAlt : this.key?.remoteJid);
			},
		},
		isGroup: {
			get() {
		...
		sender: {
			get() {
				return this.conn?.getJid(this?.key?.participantAlt || this?.participant || this?.key?.participant || this.chat);
			},
			enumerable: true,
		},
		fromMe: {
		...
		text: {
			get() {
				const msg = this.msg;
				const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption || msg?.selectedId || msg?.contentText || msg?.selectedDisplayText || msg?.hydratedTemplate?.hydratedContentText || '';
